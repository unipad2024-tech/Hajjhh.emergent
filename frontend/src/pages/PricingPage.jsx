...
export default function PricingPage() {
  ...
  const handleUpgrade = async (planId) => {
    if (!currentUser) {
      toast.info("سجّل دخولك أولاً للاشتراك");
      navigate("/login");
      return;
    }
    if (currentUser.subscription_type === "premium") {
      toast.info("أنت مشترك مميز بالفعل!");
      return;
    }
    setLoading(planId);
    try {
  ...
  return (
    <div className="min-h-screen px-4 py-8" style={DARK_BG}>
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        <button
          onClick={() => navigate("/")}
          className="text-secondary/50 hover:text-secondary mb-8 flex items-center gap-2 transition-colors text-sm"
        >
          ← رجوع
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
          {[
            ...
            <div
              key={tier.label}
              className={`rounded-2xl p-5 border ${!tier.isFree ? "border-secondary/60 bg-primary/80" : "border-secondary/15 bg-primary/30"}`}
              ...
              <ul className="space-y-2">
                {tier.features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm">
                    <span className={tier.isFree ? "text-secondary/30" : "text-secondary"}>
                      {tier.isFree ? "○" : "✓"}
                    </span>
        ...
        {/* Pricing cards */}
        {isPremium ? (
          <div className="text-center bg-primary/60 border border-secondary/40 rounded-2xl p-8 animate-scale-in">
            <div className="text-5xl mb-3">✓</div>
            <h2 className="text-secondary font-black text-2xl mb-2">أنت مشترك مميز!</h2>
            <p className="text-secondary/60 text-sm mb-4">
              تنتهي اشتراكك: {currentUser?.subscription_expires_at ? new Date(currentUser.subscription_expires_at).toLocaleDateString("ar-SA") : "—"}
            </p>
            <button
              onClick={() => navigate("/")}
              className="bg-secondary text-primary font-black px-8 py-3 rounded-full hover:scale-105 transition-all"
            >
              العب الحين
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            {PLANS.map((plan) => (
              <div
                key={plan.id}
                className="relative bg-primary/70 border border-secondary/30 rounded-2xl p-6 hover:border-secondary/60 transition-all duration-300"
                style={{ boxShadow: "0 4px 30px rgba(0,0,0,0.3)" }}
              >
                {plan.badge && (
                  <span className="absolute -top-3 right-4 bg-secondary text-primary text-xs font-black px-3 py-1 rounded-full">
                    {plan.badge}
                  </span>
                )}
                <h3 className="text-secondary font-black text-xl mb-1">{plan.name}</h3>
                <div className="flex items-end gap-1 mb-4">
                  <span className="text-secondary text-3xl font-black">{plan.price}</span>
                ...
                <button
                  ...
                  className="w-full bg-secondary text-primary font-black py-3 rounded-full hover:scale-105 transition-all disabled:opacity-50"
                  style={{ boxShadow: "0 0 20px rgba(241,225,148,0.2)" }}
                >
                  {loading === plan.id ? "جاري التحويل..." : "اشترك الحين"}
                </button>
              </div>
            ))}
          </div>
        ...
        <p className="text-center text-secondary/30 text-xs mt-8">
          الدفع آمن ومشفّر. يمكنك إلغاء الاشتراك في أي وقت.
        </p>
      </div>
    </div>
  );
