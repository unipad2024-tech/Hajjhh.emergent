        ...
        <div className="bg-primary/60 border border-secondary/25 rounded-3xl p-8 backdrop-blur-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input data-testid="signup-email-input" type="email" value={form.email}
                onChange={(e) => setForm({ ...form, email: e.target.value })}
                placeholder="example@email.com" dir="ltr"
                className="w-full bg-black/30 border-2 border-secondary/20 focus:border-secondary text-secondary placeholder:text-secondary/25 px-4 py-3 rounded-xl outline-none transition-all" />
            </div>
            <div>
              <label className="text-secondary/70 text-xs font-bold block mb-1">اسم المستخدم</label>
              <input data-testid="signup-username-input" type="text" value={form.username}
                onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="اسمك في اللعبة"
                className="w-full bg-black/30 border-2 border-secondary/20 focus:border-secondary text-secondary placeholder:text-secondary/25 px-4 py-3 rounded-xl outline-none transition-all" />
            </div>
            <div>
